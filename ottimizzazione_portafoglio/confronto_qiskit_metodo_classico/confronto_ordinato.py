# TO DO: test con diversi ottimizzatori, numero di asset diversi. Riporta tutto nella tabella
# cicla anche per bound diversi
...
def main_qiskit(bounds_):
    ...
    return dataframe_combinazioni_fval, n_comb_vqe, ry.num_qubits, ry.depth(), toc_qiskit
...
def main_classico(bounds_):
    ...
    n_comb_classico = conta_combinazioni_classiche(espandi_bounds(BOUNDS))
    return df_classico_esaustivo, n_comb_classico, toc_classico

# PRINTO I RISULTATI
def risultati(dataframe_combinazioni_fval, df_classico_esaustivo):
    ...
    return df_classico_esaustivo.iloc[0].h - dataframe_combinazioni_fval.iloc[0].fval
...
# confronto diversi parametri

# lista_ottimizzatore_in_uso = []
...
for ott in LISTA_OTTIMIZZATORI:
    ...
    for NUMERO_ASSET in range(2, 8): 
        ...
        dataframe, lista_prezzi, log_apprezzamenti_quotidiani, cov_matrix, medie_diversi_er, ann_sd, assets, num_assets = elementi_utili(STRINGA_PERCORSO_CSV, NUMERO_ASSET, PREZZO_ASSET_1, PREZZO_ASSET_2, PREZZO_ASSET_3, PREZZO_ASSET_4, PREZZO_ASSET_5, PREZZO_ASSET_6, PREZZO_ASSET_7)
        vincoli_acquisto = slicing_bounds(BOUNDS, NUMERO_ASSET)
        df_vqe, numero_combinazioni_vqe, numero_qubit, depth_circuito, tempo_qiskit = main_qiskit(vincoli_acquisto)
...
print('Bisogna creare la tabella sistemata a partire dalle liste')