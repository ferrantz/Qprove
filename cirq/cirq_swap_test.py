...
class SwapTest:
    ...
    def build_circuit(self, input_1 = None, input_2 = None, input_1_transforms = None, input_2_transforms = None):
        ...
        # trasformata di Hadamard sull'ancilla
        self.circuit.append(cirq.H(self.ancilla_qubit))
        if self.measure:
...
def main(prepare_input_states = True, input_state_dim = 4, input_1_transforms = [cirq.H], input_2_transforms = [cirq.I], measure = True, copies = 1000):
    ...
    st.build_circuit(input_1_transforms = input_2_transforms, input_2_transforms = input_2_transforms)
    prob_0, dot_product_sq = st.simulate()
    print(f'Probabilità dello zero state {prob_0}')
    print(f'Sq del dot product {dot_product_sq}')
